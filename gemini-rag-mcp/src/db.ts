 ...
 * Wraps D1 database operations for Gemini RAG MCP
 */
...
export async function updateUserPlan(db: D1Database, userId: string, plan: string): Promise<void> {
  await db
...
export async function deleteUser(db: D1Database, userId: string): Promise<void> {
  ...
  await db
    ...
    .run();
  await db
    .prepare('UPDATE gemini_connections SET status = ? WHERE user_id = ?')
    .bind('deleted', userId)
    .run();
}

// ============================================
// Gemini Connection Operations
// ============================================
...
export async function createConnection(
  ...
  defaultCorpusId: string | null = null
): Promise<GeminiConnection> {
  const id = generateUUID();
  const now = new Date().toISOString();

  await db
    .prepare(
      `INSERT INTO gemini_connections (id, user_id, name, gemini_api_key_encrypted, default_corpus_id, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, 'active', ?, ?)`
    )
    .bind(id, userId, name, geminiApiKeyEncrypted, defaultCorpusId, now, now)
    .run();
  ...
  return {
    ...
    user_id: userId,
    name,
    gemini_api_key_encrypted: geminiApiKeyEncrypted,
    default_corpus_id: defaultCorpusId,
    status: 'active',
    last_tested_at: null,
    created_at: now,
    updated_at: now,
...
export async function getConnectionsByUserId(db: D1Database, userId: string): Promise<GeminiConnection[]> {
  const result = await db
    .prepare('SELECT * FROM gemini_connections WHERE user_id = ? AND status != ?')
    .bind(userId, 'deleted')
    .all<GeminiConnection>();
  return result.results || [];
}

export async function getConnectionById(db: D1Database, id: string): Promise<GeminiConnection | null> {
  const result = await db
    .prepare('SELECT * FROM gemini_connections WHERE id = ?')
    .bind(id)
    .first<GeminiConnection>();
  return result || null;
}
...
export async function deleteConnection(db: D1Database, id: string): Promise<void> {
  ...
  await db.prepare('DELETE FROM gemini_connections WHERE id = ?').bind(id).run();
}

export async function countUserConnections(db: D1Database, userId: string): Promise<number> {
  const result = await db
    .prepare('SELECT COUNT(*) as count FROM gemini_connections WHERE user_id = ? AND status = ?')
    .bind(userId, 'active')
    .first<{ count: number }>();
...
export async function getAllUsers(
  ...
  const dataResult = await db
    .prepare(`SELECT id, email, plan, status, is_admin, created_at, updated_at FROM users${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`)
...
export async function getAdminStats(db: D1Database): Promise<{
  ...
  const [usersResult, todayUsage] = await Promise.all([
